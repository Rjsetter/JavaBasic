...
/**
 * 二叉搜索树的封装类
 * 该类中，如果有重复值，插入到右子节点，查找时也只返回第一个找到的节点
 */
public class BinaryTree {
    ...
    public Node find(int key){
        ...
        while(cur.person.getAge() != key){
            if(key<cur.person.getAge()){
                cur = cur.leftChild;   //key比当前节点小，转向左子节点
            }else{
                cur = cur.rightChild; //key比当前结点大，转向右子节点
            }
            if(cur == null){
                return null; //如果没有查到结果，搜索结束
            }
        }
        return cur;   //找到了，返回当前结点
    }

    //插入新节点
    public void insert(Node node){
        if(root == null){//树为空，则插入的结点为根节点
            root = node;
        }else{
            Node cur = root;
            while(true){
    ...
    public void preorder(Node currentRoot){
        if(currentRoot != null){
            System.out.print(currentRoot.person+"\t");
    ...
    public void inorder(Node currentRoot){
        if(currentRoot != null){
            ...
            System.out.print(currentRoot.person+"\t"); //然后访问当前节点
        ...
        while(layerIndex <= depth){
            ...
            for(int i = 0;i<currentLayerNodes.size();i++){
                ...
                if(node == null){
                    ...
                    System.out.print("*  "+node.person+"\t");  //打印该节点
                }
